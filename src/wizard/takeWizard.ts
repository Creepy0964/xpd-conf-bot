...
test.on(media_group(), async (ctx: MediaGroupContext<MyContext>) => {
    ...
    for (const media of ctx.update.media_group) {
        if ('photo' in media) {
            if('caption' in media) mediaArray.push({type: 'photo', media: media.photo[0].file_id, caption: `\`${media.caption.replace(/([_*[\]()~`>#\+\-=|{}.!])/g, "\\$1")}\n\n#тейк | @xpdforum_takes_bot | @xpdforum\``, parse_mode: 'MarkdownV2'})
            else mediaArray.push({type: 'photo', media: media.photo[0].file_id});
        }
        if ('video' in media) {
            if('caption' in media) mediaArray.push({type: 'video', media: media.video[0].file_id, caption: `\`${media.caption.replace(/([_*[\]()~`>#\+\-=|{}.!])/g, "\\$1")}\n\n#тейк | @xpdforum_takes_bot | @xpdforum\``, parse_mode: 'MarkdownV2'})
            else mediaArray.push({type: 'video', media: media.video[0].file_id});
        }
    }

    await bot.telegram.sendMediaGroup(-1002249581112, mediaArray);
    await bot.telegram.sendMessage(-1002249581112, `Автор медиагруппы: ${author}`, {parse_mode: 'MarkdownV2'});
...
export const takeWizard = new Scenes.WizardScene<MyContext>('take',
    async (ctx) => {
        ...
        await ctx.sendMessage(`Напишите Ваш тейк.\n\nПримечание: мы публикуем тейки ТОЛЬКО в том случае, если его содержание напрямую относится к теме РЛ. Это значит, что тейки с побочными темами (аффективные расстройства, нейроотличия и т.д.), венты и т.п., не публикуются. Надеемся на понимание :)`);
